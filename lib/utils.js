...
function schema(bookfile, backend, charset) {
  backend = backend || 'sqlite';
  const env = { MDB_JET3_CHARSET: charset || 'cp1256' };
  const cmd = `mdb-schema ${bookfile} ${backend}`;

  return size(bookfile).then(maxBuffer => exec(cmd, { env, maxBuffer }));
};

function tables(bookfile, backend, charset) {
  backend = backend || 'sqlite';
  const env = { MDB_JET3_CHARSET: charset || 'cp1256' };
...
function extract(bookfile, table, backend, charset) {
  backend = backend || 'sqlite';
  const env = { MDB_JET3_CHARSET: charset || 'cp1256' };
  let cmd;
  if (backend === 'csv') {
...
function extractAll(bookfile, backend, charset) {
  backend = backend || 'sqlite';
  return tables(bookfile, backend, charset)
    .map((table) => extract(bookfile, table, backend, charset));
}

function extractJson(bookfile, table) {
  const env = { MDB_JET3_CHARSET: 'cp1256' };
...
module.exports = {
  schema, tables, extract, extractAll, extractJson
};

...
function schema(opts = {}) {
  ...
  if (opts.config.dialect === 'sqlite3' || opts.config.client === 'sqlite3') {
    opts.config.pool = {
...
function install(book, opts = {}) {
  ...
  return dumpJson(book)
    .then((data) => {
      ...
      function createQuery(knex, uninstall) {
        ...
        const categoryRemove = knex('category')
          .where({ name: category }).del();

        const kitabRemove = knex('kitab')
          .where({ name: bookName, category_id: categorySelect }).del();

        const authorRemove = knex('author')
          .where({ name: author }).del();
        ...
        if (!uninstall) {
          ...
          return [authorRemove, kitabRemove];
        }

      }

      return dump
        ? wrap(createQuery(knex, uninstall).map(q => q.toString()), true)
        : knex.transaction((trx) => Promise.reduce(createQuery(trx, uninstall), (acc) => acc)).then(() => knex);
    });
}
